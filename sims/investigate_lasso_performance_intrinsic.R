# simulation comparing extrinsic, intrinsic selection vs lasso in a more
# adversarial situation for the lasso
...
beta_0[active_set_with_intercept] <- 1

# set up fixed parameters
per_comparison_alpha <- 0.1
sl_threshold <- 0.2
# match SL rank selection with SS q's; note q ~ sqrt(5(p / 2))
sl_rank <- round(sqrt(5 * current_dynamic_args$p / 2))
kf_fdr <- 0.2
...
target_specificity <- set_specificity(n = current_dynamic_args$n, ns = ns,
                                      ...
                                      max_spec = 0.75)
gfwer_k <- max(ceiling((1 - target_specificity) * (current_dynamic_args$p - (length(active_set)))), 2) + 1
# we can also specify a PFP of interest; want more control at higher sample size
# note that at n = 200, we want the maximum PFP
s0 <- length(active_set)
pfp_q <- gfwer_k / ((s0) / current_dynamic_args$p * (sqrt(current_dynamic_args$n) / sqrt(200)) + gfwer_k)
...
system.time(sim_output <- sapply(1:args$nreps_per_job, function(i)
  do_one(indx = i + args$nreps_per_job * (current_dynamic_args$mc_id - 1),
         ...
         data_only = args$est_truth, active_set = active_set),
  simplify = FALSE)
)
